...
def load_file(filename):
    ...
    # if the image file consists four-channel format; R, G, B, and A (instead of RBG), we will remove the the last column which containe Alpha values
    if num_channel == 4:
...
def find_centroids_manhattan(pixels, k=2):
    ...
    while True:
        ...
        # each data point will be assigned to its closest cluster center in terms of l1(Manhattan) distance
        for i in range(len(pixels)):
            dist = np.sum(abs(pixels[i] - init_cent), axis=1)
            classes[i] = np.argmin(dist)
        
        # each centroid will be moved to the median of all data points that belong to the centroid
        centroid = []
        for n in range(k):
...
for filename in im_filenames:
    ...
    classes, centroid, iter_count, elapsed_sec = find_centroids_manhattan(pixels=pixels, k=k)

    # just in case if the k is too large, we will decrement k value utill we have no empty clusters
    # in this case, we might want to calculate the number of clusters(num_clusters) to see how many clusters we have 
